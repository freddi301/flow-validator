...
export class AsyncObjectType<S: {[key: string]: AsyncType<any>}, T: $ObjMap<S, <F>(v: AsyncType<F>) => F>> extends AsyncType<T> {
  schema: S;
...
export function asyncObject<S: {[key: string]: AsyncType<any>}>(s: S): AsyncObjectType<S, $ObjMap<S, <F>(v: AsyncType<F>) => F>> {
  const os = new AsyncObjectType(s, async (v) => {
    ...
    await Promise.all(keys.map(async (key) => {
      try { result[key] = await s[key].parse(o[key]); } catch (e) {
