...
export class AsyncVObjectExactType<S: {[key: string]: AsyncVType<any>}, T: $ObjMap<S, <F>(v: AsyncVType<F>) => F>> extends AsyncVType<T> {
  schema: S;
...
export function asyncVobjectExact<S: {[key: string]: AsyncVType<any>}>(s: S): AsyncVObjectExactType<S, $Exact<$ObjMap<S, <F>(v: AsyncVType<F>) => F>>> {
  const oes = new AsyncVObjectExactType(s, async (v) => {
    ...
    await Promise.all(keys.map(async (key) => {
      ...
      else try { await s[key].validate(o[key]); } catch (e) { if (e instanceof ValidationError) errors[key] = e; else throw e; }
    }));
