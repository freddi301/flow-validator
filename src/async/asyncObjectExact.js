...
export class AsyncObjectExactType<S: {[key: string]: AsyncType<any>}, T: $ObjMap<S, <F>(v: AsyncType<F>) => F>> extends AsyncType<T> {
  schema: S;
...
export function asyncObjectExact<S: {[key: string]: AsyncType<any>}>(s: S): AsyncObjectExactType<S, $Exact<$ObjMap<S, <F>(v: AsyncType<F>) => F>>> {
  const oes = new AsyncObjectExactType(s, async (v) => {
    ...
    await Promise.all(keys.map(async (key) => {
      ...
      else try { result[key] = await s[key].parse(o[key]); } catch (e) {
