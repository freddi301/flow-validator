...
export class AsyncVMappingType<K, V> extends AsyncVType<{[key: K]: V}> {
  keys: AsyncVType<K>;
  values: AsyncVType<V>;
  constructor(keys: AsyncVType<K>, values: AsyncVType<V>, validate: (value: mixed) => Promise<{[key: K]: V}>) {
...
export function asyncVmapping<K: string, V>(keys: AsyncVType<K>, values: AsyncVType<V>): AsyncVMappingType<K, V> {
  const m = new AsyncVMappingType(keys, values, async (v) => {
    ...
    await Promise.all(ks.map(async (key) => {
      const value = o[key];
      let ke; let ve;
      try { await keys.validate(key); } catch (e) { if (e instanceof ValidationError) ke = e; else throw e; }
      try { await values.validate(value); } catch (e) { if (e instanceof ValidationError) ve = e; else throw e; }
