...
async function apiOk(x) {
  return x;
}
async function apiFail(x, error) {
...
describe("asyncArrayOf", () => {
  it("resolves on expected type", async () => {
    ...
    expect(
      await asyncArrayOf(string.async().to(apiOk)).parse(["hello", "bye"])
    ).to.deep.equal(["hello", "bye"]);
  });
  it("rejects on invalid type", async () => {
    const data = ["hello", Date];
    try {
      await asyncArrayOf(string.async()).parse(data);
      throw new Error();
    } catch (e) {
      expect(e).to.be.instanceof(ValidationError);
      expect(e.toJSON()).deep.equal({
        expected: { name: "arrayOf" },
        got: data,
  ...
  it("rejects if .to() rejects", async () => {
    ...
    try {
      await asyncArrayOf(string.async().to(apiFail)).parse(data);
      throw new Error();
    } catch (e) {
      expect(e).to.be.instanceof(ValidationError);
      expect(e.toJSON()).deep.equal({
...
describe("asyncVarrayOf", () => {
  it("resolves on expected type", async () => {
    ...
    expect(
      await asyncVarrayOf(string.Vasync().Vrefine(apiOk)).validate(data)
    ).to.deep.equal(data);
    expect(
      await asyncVarrayOf(string.Vasync().Vrefine(apiOk)).validate(data)
    ).to.equal(data);
  ...
  it("rejects on invalid type", async () => {
    const data = ["hello", Date];
    try {
      await asyncVarrayOf(string.Vasync()).validate(data);
      throw new Error();
    } catch (e) {
      expect(e).to.be.instanceof(ValidationError);
      expect(e.toJSON()).deep.equal({
  ...
  it("rejects if .refine() rejects", async () => {
    const data = ["hello", "bye"];
    try {
      await asyncVarrayOf(string.Vasync().Vrefine(apiFail)).parse(data);
      throw new Error();
    } catch (e) {
      expect(e).to.be.instanceof(ValidationError);
      expect(e.toJSON()).deep.equal({
